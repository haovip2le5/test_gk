...
4. [Quản Lý Câu Hỏi (Mới)](#quản-lý-câu-hỏi-mới)
5. [Thay Đổi Dữ Liệu Test](#thay-đổi-dữ-liệu-test)
6. [Thêm Endpoints Mới](#thêm-endpoints-mới)
7. [Cơ Sở Dữ Liệu (In-Memory)](#cơ-sở-dữ-liệu-in-memory)
8. [Debug & Logging](#debug--logging)
9. [Cấu Hình Backend](#cấu-hình-backend)
...
### 4. **Quản Lý Câu Hỏi (Question Management)** ⭐ MỚI
...
{
  "success": true,
  "message": "Đã thêm câu hỏi mới với ID 9",
  "question": {
    "id": 9,
    "question": "Câu hỏi mới của bạn?",
    "options": ["Lựa chọn A", "Lựa chọn B", "Lựa chọn C", "Lựa chọn D"],
...
Sửa câu hỏi (tùy chọn - chỉ cập nhật các trường cần thiết)

**Request (Ví dụ 1 - Sửa toàn bộ):**
```json
{
  "question": "Câu hỏi đã sửa?",
  "options": ["Lựa chọn mới 1", "Lựa chọn mới 2", "Lựa chọn mới 3", "Lựa chọn mới 4"],
  "correct_answer": "Lựa chọn mới 3"
}
```

**Request (Ví dụ 2 - Chỉ sửa câu hỏi):**
```json
{
  "question": "Câu hỏi đã sửa?"
}
...
{
  "success": true,
  "message": "Đã cập nhật câu hỏi ID 1",
  "question": {
    "id": 1,
    "question": "Câu hỏi đã sửa?",
    "options": ["Python", "JavaScript", "Java", "C#"],
...
**Request:**
```
DELETE http://localhost:8000/admin/questions/1
```
...
curl -X POST http://localhost:8000/admin/questions \
  ...
  -d '{
    ...
    "options": ["HyperText Markup Language", "High Tech Modern Logic", "Home Tool Markup List", "Hyper Transfer Markup Link"],
    "correct_answer": "HyperText Markup Language"
  }'
```

#### 2. **Sửa Câu Hỏi với Postman**
...
```json
{
  "question": "React là thư viện của ngôn ngữ lập trình nào?",
...
```bash
# Với Curl:
curl -X DELETE http://localhost:8000/admin/questions/1

# Hoặc với PowerShell Windows:
Invoke-WebRequest -Uri http://localhost:8000/admin/questions/1 -Method DELETE
```

#### 4. **Thêm Nhiều Câu Hỏi Chuỗi**

```bash
#!/bin/bash
# Lưu vào file: add_questions.sh
...
for i in {1..3}; do
  ...
  echo "Đã thêm câu hỏi $i"
done
```

### Lưu Ý

- ✅ **Toàn bộ 4 lựa chọn bắt buộc** - Mỗi câu hỏi phải có đúng 4 lựa chọn
- ✅ **Đáp án phải nằm trong options** - correct_answer phải là một trong 4 lựa chọn
- ✅ **ID tự động** - Khi thêm câu hỏi mới, hệ thống tự sinh ID tiếp theo
- ⚠️ **Dữ liệu tạm thời** - Câu hỏi sẽ mất khi restart server (in-memory database)
- 📝 **Chỉ có 8 câu hỏi mặc định** - Sau khi xóa hết, tổng số câu hỏi trong quiz sẽ giảm
